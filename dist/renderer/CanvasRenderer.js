import { ROW_HEADER_WIDTH, COLUMN_HEADER_HEIGHT, GRID_LINE_COLOR, HEADER_BACKGROUND, FONT, TEXT_COLOR } from "../utils/Constants.js";
/**
...
export class CanvasRenderer {
    ...
    drawColumnHeaders(startColumn, endColumn, scrollLeft) {
        const selection = this.selectionManager.getSelection();
        this.ctx.fillStyle = HEADER_BACKGROUND;
        this.ctx.fillRect(0, 0, this.canvas.width, COLUMN_HEADER_HEIGHT);
        ...
        for (let col = startColumn; col <= endColumn; col++) {
            ...
            this.ctx.strokeRect(x, 0, currentWidth, COLUMN_HEADER_HEIGHT);
            ...
            this.ctx.fillText(header, x + currentWidth / 2, COLUMN_HEADER_HEIGHT / 2);
        }
    }
    drawRowHeaders(startRow, endRow, scrollTop) {
        const selection = this.selectionManager.getSelection();
        this.ctx.fillStyle = HEADER_BACKGROUND;
        this.ctx.fillRect(0, 0, ROW_HEADER_WIDTH, this.canvas.height);
        this.ctx.strokeStyle = GRID_LINE_COLOR;
        this.ctx.font = FONT;
        this.ctx.textAlign = "center";
        this.ctx.textBaseline = "middle";
        for (let row = startRow; row <= endRow; row++) {
            ...
            this.ctx.strokeRect(0, y, ROW_HEADER_WIDTH, currentHeight);
            this.ctx.fillStyle = TEXT_COLOR; //Otherwise the letters become blue too
            this.ctx.fillText(String(row + 1), ROW_HEADER_WIDTH / 2, y + currentHeight / 2);
        }
    }
    drawGrid(startRow, endRow, startColumn, endColumn, scrollTop, scrollLeft) {
        ...
        for (let row = startRow; row <= endRow; row++) {
            ...
            for (let col = startColumn; col <= endColumn; col++) {
                const x = ROW_HEADER_WIDTH + this.rowColumnManager.getColumnX(col) - scrollLeft;
                const currentWidth = this.rowColumnManager.getColumnWidth(col);
                this.ctx.strokeRect(x, y, currentWidth, currentHeight);
            }
        }
    }
    drawCellContents(startRow, endRow, startColumn, endColumn, scrollTop, scrollLeft) {
        ...
        for (let row = startRow; row <= endRow; row++) {
            ...
            for (let col = startColumn; col <= endColumn; col++) {
                ...
                this.ctx.save();
                this.ctx.beginPath();
                this.ctx.rect(x, y, currentWidth, currentHeight);
                this.ctx.clip();
                this.ctx.fillText(String(value), x + 5, y + currentHeight / 2);
                this.ctx.restore();
            }
        }
    }
