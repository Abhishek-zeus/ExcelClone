import { HANDLE_SIZE, ROW_HEADER_WIDTH, COLUMN_HEADER_HEIGHT, GRID_LINE_COLOR, FONT, TEXT_COLOR } from "../utils/Constants.js";
...
export class CanvasRenderer {
    constructor(canvas, dataModel, selectionManager, viewport, rowColumnManager) {
        this.canvas = canvas;
    ...
    render(startRow, endRow, startColumn, endColumn, scrollTop, scrollLeft) {
        this.clearCanvas();
    ...
    drawGrid(startRow, endRow, startColumn, endColumn, scrollTop, scrollLeft) {
        ...
        for (let row = startRow; row <= endRow; row++) {
            ...
            for (let col = startColumn; col <= endColumn; col++) {
                ...
                this.ctx.strokeRect(currentX, currentY, currentWidth, currentHeight);
                currentX += currentWidth;
            }
            currentY += currentHeight;
        }
    }
    drawCellContents(startRow, endRow, startColumn, endColumn, scrollTop, scrollLeft) {
        this.ctx.font = FONT;
        ...
        for (let row = startRow; row <= endRow; row++) {
            ...
            for (let col = startColumn; col <= endColumn; col++) {
                ...
                if (value !== "") {
                    ...
                    this.ctx.save();
                    this.ctx.beginPath();
                    this.ctx.rect(currentX, currentY, currentWidth, currentHeight);
                    this.ctx.clip();
                    this.ctx.fillText(String(value), currentX + 5, currentY + currentHeight / 2);
                    this.ctx.restore();
                }
                currentX += currentWidth;
            }
    ...
    drawSelection() {
        ...
        for (let row = selection.startRow; row <= endRow; row++) {
            const currentHeight = this.rowColumnManager.getRowHeight(row);
            let currentX = ROW_HEADER_WIDTH + this.rowColumnManager.getColumnX(selection.startColumn) -
                this.viewport.getScrollLeft();
            for (let col = selection.startColumn; col <= endCol; col++) {
                const currentWidth = this.rowColumnManager.getColumnWidth(col);
                this.ctx.fillRect(currentX, currentY, currentWidth, currentHeight);
                currentX += currentWidth;
            }
            currentY += currentHeight;
        }
        ...
        const right = ROW_HEADER_WIDTH + this.rowColumnManager.getColumnX(endCol) + this.rowColumnManager.getColumnWidth(endCol) - this.viewport.getScrollLeft();
        const bottom = COLUMN_HEADER_HEIGHT + this.rowColumnManager.getRowY(endRow) + this.rowColumnManager.getRowHeight(endRow) - this.viewport.getScrollTop();
